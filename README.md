...
- Simple header with the gallery title only—search and navigation icons have
  been removed for a cleaner look.
...
2. **Adding photos:** drop any JPEG or PNG files into the `images` directory and
   commit them.  Thanks to Jekyll’s Liquid templating, `index.html` automatically
   loops through that folder to create a photo card for each image.  Captions are
   generated from file names (hyphens and underscores become spaces and the first
   word is capitalised), so you don’t need to edit HTML when adding new pictures.
